...
# -*- coding: utf-8 -*-
"""
Job Search Page for Pennsylvania Employment Dashboard
===================================================

This page provides job search functionality with integration to BLS SOC codes,
O*NET data, and future job posting data.
...
from utils.job_title_mapping import find_soc_code, get_all_job_titles, get_categories
from utils.data_loader import load_pa_occupation_data, load_bls_data, load_job_listings_data
...
def show_job_search():
    ...
    with st.spinner("Loading data..."):
        ...
        job_listings = load_job_listings_data()
    ...
    with col1:
        ...
        search_input = st.text_input(
            "Enter Job Title:",
            value=st.session_state.get('job_search_input', ''),
            placeholder="e.g., Software Developer, Data Analyst, Cybersecurity Engineer",
            help="Type a job title to search for market analysis"
        )
...
def perform_job_search(job_title, pa_data, bls_data, job_listings):
    ...
    if soc_info:
        ...
        if bls_data is not None:
            bls_match = search_bls_data(soc_info["soc_code"], bls_data)
            results["bls_data"] = bls_match
        
        # Search job listings
        if not job_listings.empty:
            job_matches = search_job_listings(job_title, job_listings)
...
def search_bls_data(soc_code, bls_data):
...
def display_search_results(results, search_input):
    ...
    if results["job_listings"]:
        display_job_listings(results["job_listings"])
        ...
        with st.expander(f"{i}. {job['title']} at {job['company']} (Match Score: {job['match_score']})"):
            ...
            with col1:
                ...
                if job['skills'] and len(job['skills']) > 0:
                    ...
                    skills_text = ", ".join(job['skills'][:5])  # Show first 5 skills
                    if len(job['skills']) > 5:
                        skills_text += f" (+{len(job['skills']) - 5} more)"
                        st.write(skills_text)
                        
                        # Use session state to manage expand state
                        expand_key = f"skills_expand_{i}_{hash(str(job['skills']))}"
                        if st.button("📖 View All Skills", key=f"job_listing_{expand_key}"):
                            st.session_state[expand_key] = True
                        
                        # Show expanded content if state is True
                        if st.session_state.get(expand_key, False):
                            all_skills = ", ".join(job['skills'])
